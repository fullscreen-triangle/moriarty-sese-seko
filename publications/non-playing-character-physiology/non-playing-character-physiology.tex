...
pass, sometimes with a token acknowledgement, sometimes with none. This
selectivity is the response side of the synthetic-agent problem~\cite{%
russellnorvig2020,simon1996}.
...
(standard convex analysis~\cite{rockafellar1970,boydvandenberghe2004,%
nesterov2018}):
$\drive(y)\ge\drive(x)+\nabla\drive(x)^\top(y-x)+\tfrac{\lam}{2}
\norm{y-x}^2$ for all $x,y$. Its behaviour, absent interaction, follows
the descent $\dot x=-\nabla\drive(x)$~\cite{khalil2002,%
...
Before defining the consistency graph we must say what \emph{gives} the
agent a transport structure at all: the graph must be grounded in the
agent's internal constitution, not introduced fully formed.

\begin{axiom}[Disposition components carry transports]
\label{ax:transport}
The agent's disposition components stand in relations of internal
dependence: shifting one component induces, through the agent's own
internal structure, a determinate shift in each dependent component. Each
such directed dependence carries a \emph{transport}---the shift induced per
unit of the driving component. Formally, the agent's internal structure
supplies, for each ordered pair of dependent components $(i,j)$, a transport
$\theta_{ij}\in\R^d$; the consistency graph $H$ of \Cref{def:coherence} is
the formalisation of this transport structure.
\end{axiom}

\begin{remark}[The consistency graph is the self-graph carrying its costs
as transports]
\label{rem:H-grounded}
The transport structure of \Cref{ax:transport} is not a new object bolted
onto the agent but the companion account's self-graph seen dynamically. In
that account the agent is a finite weighted graph of internal distinctions
whose edges carry the \emph{cost} of holding two parts apart; a shift in one
part is transmitted to an adjacent part through exactly that separation, and
the induced shift per unit drive is the transport $\theta_{ij}$. The
consistency graph $H$ shares the self-graph's vertices (disposition
components) and its edges (internal separations), and reads each separation
cost as the magnitude of a directed transport. Holonomy---the failure of
transports to sum to zero around a cycle---is then the dynamical signature
of the same internal boundary costs the companion account derives, so $H$
arises from the agent's constitution rather than alongside it. This is what
grounds the pre-commitment coherence test (\Cref{thm:cohtest}) and the
refusal corollary (\Cref{cor:refuse}): both read structure the agent already
has.
\end{remark}
...
cycle is two routes that disagree. This is the graph- and
geometry-theoretic notion of holonomy and cycle
space~\cite{bondymurty2008,diestel2017,hatcher2002,kobayashinomizu1963}.
Coherence as vanishing holonomy is therefore not a special assumption but
the general form of internal consistency, and---crucially for \Cref{sec:coherence}---it is checkable
...
\begin{remark}[Why the floor is constitutive of response]
\label{rem:floor-earned}
The floor is not an independent stipulation but is built into what a
response \emph{is}. A zero-cost response deposits nothing and changes
nothing---it leaves the agent's disposition, expended attention, and
committed count exactly as they were. But a change that changes nothing is
not a response; it is the \emph{absence} of response, indistinguishable from
declining. Therefore every genuine response---anything that counts as the
agent having replied at all---costs at least some positive $\floorc>0$. The
floor is thus the trace that distinguishes responding from doing nothing,
and \Cref{ax:floor} records the least such trace across the repertoire.
\end{remark}
...
interval, $\sum_{\resp\in R}\cost(\resp)\le\att$. This finiteness is the
formal counterpart of bounded rationality~\cite{simon1955}.
\end{axiom}
...
if $\gain(\resp;x)>0$. Reading the residual $D$ as a value-to-go and the
gain as its reduction places the policy in the setting of reward and
value~\cite{sutton2018}, while the residual itself is an information-like
gap in the sense of~\cite{shannon1948,coverthomas2006}.
\end{definition}
...
The responses of \Cref{def:response} are drawn from a \emph{finite}
repertoire: they are discrete, all-or-nothing, not divisible at will. The
value-maximising selection is therefore an integer knapsack, which is
NP-hard in general~\cite{martello1990,kellerer2004,cormen2009}. Two honest positions are available, and we take both:
the threshold rule is \emph{exactly} optimal when the repertoire has the
additive structure below, and otherwise optimal for the continuous
relaxation and within one boundary item of the true integer optimum.

\begin{definition}[Floor-additive repertoire]
\label{def:additive}
A response repertoire is \emph{floor-additive} if every response decomposes
into acknowledgement-sized units of cost $\floorc$ with proportional gain:
each $\resp$ has $\cost(\resp)=k_\resp\floorc$ for some integer
$k_\resp\ge1$, and gain accrues linearly across its units, so that
committing $k_\resp$ units of the cheapest floor-cost response secures the
same gain density as $\resp$. Equivalently, cost is additive and gain is
additive over floor-sized units.
\end{definition}

\begin{theorem}[Optimal policy is a threshold on gain density]
\label{thm:threshold}
Order relevant interactions by \emph{gain density}
$\gain(\resp;x)/\cost(\resp)$ and commit them in decreasing order until the
budget $\att$ is exhausted; equivalently, there is a price $p^\star\ge0$
such that the agent responds to an interaction iff its best admissible
response has gain density $\ge p^\star$, and ignores the rest, with
$p^\star$ rising as the budget $\att$ falls or as relevant arrivals grow.
Then:
\begin{enumerate}[label=\textup{(\roman*)},leftmargin=2.2em]
\item for a floor-additive repertoire (\Cref{def:additive}) the threshold
rule is \emph{exactly} value-maximising among all integer selections; and
\item for a general finite repertoire it is optimal for the continuous
relaxation and its value is within one response of the integer optimum---the
gap is at most the gain of a single boundary item.
\end{enumerate}
\end{theorem}

\begin{proof}
The selection problem is: maximise $\sum_{\resp\in R}\gain(\resp)$ subject
to $\sum_{\resp\in R}\cost(\resp)\le\att$. Sorting by gain density and
taking greedily until the budget is spent is optimal for the
\emph{continuous} relaxation, and the Lagrangian dual assigns a single
multiplier $p^\star$---the shadow price of attention---such that an item is
selected iff its gain density exceeds $p^\star$; tightening $\att$ or adding
higher-density competitors raises the binding $p^\star$.

(i) When the repertoire is floor-additive, every response is an integer
number of floor-sized units of equal gain density, so the feasible set is
closed under splitting a response into its units and the continuous optimum
is attained at an integer point: the greedy threshold fills the budget in
whole floor units with no fractional remainder, hence is exactly optimal
among integer selections.

(ii) In general the integer (all-or-nothing) knapsack differs from its
relaxation only in the single item that the continuous solution would split
at the budget boundary; rounding that item out loses at most its own gain.
So the threshold policy's value is within one response of the true integer
optimum, and the qualitative threshold structure and shadow price $p^\star$
are unchanged. We do not claim exact integer optimality outside the
floor-additive case; the bounded gap is the honest statement.
\end{proof}
...
its gain density clears $p^\star$. The boundary item of
\Cref{thm:threshold}(ii) is precisely this floor-cost response: for a
floor-additive repertoire (\Cref{def:additive}) the residual is filled
exactly, and for a general repertoire the token is the at-most-one boundary
response by which the greedy value may fall short of the integer optimum.
\end{proof}
...
\begin{remark}[Two quantities, and what each establishes]
\label{rem:two-quantities}
\Cref{thm:irreversible} tracks two distinct quantities and it is worth being
exact about what each does. The \emph{committed count} $m$ is a timestamp;
the \emph{expended attention} is a running total of cost; \emph{neither} is
by itself a change in the disposition $x$. Read at the level of these two
alone, the claim ``a revisited disposition is never a revisited state'' is
true only because the state \emph{includes} the count---it is a consequence
of bookkeeping, and two agents at the same disposition but different counts
are distinguished by timestamp alone. That much is honest and already
enough for \Cref{rem:new-agent}. But the irreversibility is not
\emph{merely} counted, as the next proposition shows: an admissible
response also moves the disposition non-trivially.
\end{remark}

\begin{proposition}[Admissible response moves the disposition]
\label{prop:disp-moves}
Let $\resp$ be an admissible response at $x$ (purpose-advancing and
coherence-preserving, \Cref{def:relevance}). Then it changes the
disposition by a non-trivial amount bounded below in terms of the floor:
$\resp\!\cdot x\neq x$, and the drive drop $\gain(\resp;x)=\drive(x)-
\drive(\resp\!\cdot x)>0$ forces $\norm{\resp\!\cdot x-x}\ge
\sqrt{2\gain(\resp;x)/L}>0$, where $L$ is the Lipschitz constant of
$\nabla\drive$. Hence a committed admissible response deposits an
irreversible change in $x$ itself, not only in the count.
\end{proposition}

\begin{proof}
By \Cref{def:gain} admissibility requires $\gain(\resp;x)>0$, so
$\drive(\resp\!\cdot x)<\drive(x)$ and thus $\resp\!\cdot x\neq x$. For the
magnitude, an $L$-Lipschitz gradient gives the descent bound
$\drive(x)-\drive(y)\le\nabla\drive(x)^\top(x-y)+\tfrac{L}{2}\norm{x-y}^2
\le\ldots\le \tfrac{L}{2}\norm{x-y}^2$ along the improving direction, so a
drop of $\gain$ needs $\norm{\resp\!\cdot x-x}\ge\sqrt{2\gain/L}$.
Composed with the strict count monotonicity of \Cref{thm:irreversible}, the
response changes both the disposition (by $\ge\sqrt{2\gain/L}$) and the
count (by one); the disposition change makes non-return genuine, not merely
a matter of timestamp.
\end{proof}
...
\label{ax:exclusion}
Forming a new distinction and committing to a response cannot both occur at
one instant: at each instant the agent is in exactly one phase.
\end{axiom}

\begin{remark}[Phase exclusion is forced by the self-graph structure]
\label{rem:exclusion-grounded}
Exclusion is not an appeal to an undefined ``instantaneous capacity'' but a
consequence of what the two phases \emph{do} to the agent's structure.
Forming a new distinction \emph{alters the self-graph}: it changes
$(V,E,w)$---adding a part, a separation, or a cost---and thereby changes
the disposition space $\Beh$ coordinatised by that graph. Committing a
response, by contrast, \emph{applies a map} $\resp:\Beh\to\Beh$ on the fixed
disposition space (\Cref{def:response}). These cannot occur at the same
instant because construction changes the very domain $\Beh$ on which the
response map acts: a response applied while $\Beh$ is being rebuilt has no
well-defined domain and codomain---the map $\resp\!\cdot x$ is undefined
when $x$'s coordinate space is mid-revision. The two operations require the
disposition space to be, respectively, malleable and fixed at the same
instant, which is impossible. Hence exactly one phase holds at each instant.
\end{remark}
...
The suite is deterministic given its seed ($20260719$) and runs a response
floor $\floorc=1$. Across the eight prediction groups it performs $790$
individual checks and \emph{all $790$ pass};
\Cref{tab:physiology-results} records the group-level outcomes.

\begin{table}[t]
\centering
\small
\caption{Numerical validation of \emph{The Physiology of Response}. Every
one of $790$ individual checks passes (seed $20260719$, response floor
$\floorc=1$).}
\label{tab:physiology-results}
\begin{tabular}{@{}llr@{}}
\toprule
Prediction (theorem) & Checks & Outcome \\
\midrule
V1 Two-factor relevance (\Cref{thm:conjunction}) & $200$ & pass \\
V2 Inattention forced (\Cref{thm:inattention})   & $80$  & $N\le\lfloor\att/\floorc\rfloor$ \\
V3 Threshold policy (\Cref{thm:threshold})       & $80$  & single price $p^\star$ \\
V4 Token boundary (\Cref{cor:token})             & $60$  & floor-cost only \\
V5 Coherence pre-test (\Cref{thm:cohtest})       & $150$ & sign exact \\
V6 Integrity refusal (\Cref{cor:refuse})         & $100$ & declined in advance \\
V7 Irreversibility (\Cref{thm:irreversible})     & $60$  & count strictly $\uparrow$ \\
V8 Alternation (\Cref{thm:alternation})          & $60$  & phases disjoint \\
\midrule
\textbf{Total} & $\mathbf{790}$ & \textbf{all pass} \\
\bottomrule
\end{tabular}
\end{table}

Two results carry the most weight. The largest single group is V5, the
\emph{advance} coherence test: across $150$ hypothetical responses the sign
of the pre-commitment holonomy test agrees with the sign of the realised
post-response coherence margin in every case, with no error. This is the
empirical content of \Cref{thm:cohtest}---that coherence is genuinely
decidable before commitment, from local transport data alone---and it is
what makes the integrity refusals of V6 ($100/100$ tempting-but-destructive
responses declined in advance) enforceable rather than aspirational. The
second is V1: over $200$ interactions the agent answers exactly those with a
response clearing \emph{both} bars, and never one that clears only the
purpose-gain bar or only the coherence bar---confirming that relevance is
the conjunction and that neither factor alone suffices, the paper's central
thesis. The budget results (V2--V4) confirm that the committed response count
never exceeds $\lfloor\att/\floorc\rfloor$, that the value-maximising
selection is the single-price gain-density threshold, and that a small
residual budget affords only floor-cost token acknowledgements.
\Cref{fig:physiology-1,fig:physiology-2} display these results on the
explicit agents.
